...
# These mock the Anthropic client itself (per docs/security.md's mock-vs-real
# rationale: LLM calls are mocked in unit tests -- non-deterministic, costs money,
# slow) rather than making real API calls. No network access, no API key needed.


def make_provider() -> AnthropicLLMProvider:
...
def test_classify_raises_on_malformed_json_so_caller_can_fall_back():
    provider = make_provider()
    provider._client = MagicMock()
    provider._client.messages.create.return_value = fake_message("not json")
...
def test_generate_resolution_calls_the_resolution_model_with_grounded_chunks():
    provider = make_provider()
    provider._client = MagicMock()
    provider._client.messages.create.return_value = fake_message("Here is how to fix it.")
